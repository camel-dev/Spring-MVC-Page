...
function getFileInfo(fullName) {
    ...
    if (checkImageType(fullName)) {
        // 썸네일 파일 이미지 URL
        imgsrc = "/springMVCpage/file/display?fileName=" + fullName;
        // UUID_파일명.확장자 (s_ 제외 : 원본이미지)
        fileLink = fullName.substr(14);
        // 원본파일 요청 URL
        var front = fullName.substr(0, 12); // 날짜 경로
        var end = fullName.substr(14);      // 파일명(s_ 제외)
        getLink = "/springMVCpage/file/display?fileName=" + front + end;
    
    // 이미지 파일이 아닐 경우
    } else {
        // 파일 아이콘 이미지 URL
    	imgsrc = "/springMVCpage/resources/upload/files/file-icon.png";
        // UUID_파일명.확장자
        fileLink = fullName.substr(12);
        // 파일 요청 url
        getLink = "/springMVCpage/file/display?fileName=" + fullName;
    }
    // 화면에 출력할 파일명
